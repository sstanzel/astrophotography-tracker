...
children.push(table([2400, 1800, 5160], ["Section", "Filled by", "Contents"], [
  ...
  ["[future_processing]", "hand", "Short to-do lines; the tracker aggregates them across all sessions into one queryable processing to-do list, and marks the session in the work queue while any remain."],
]));
...
children.push(p("A session folder is one target, one acquisition night with one telescope and one camera. The name is four whitespace-delimited tokens — the target identifier with underscores joining its parts, the telescope, the camera, and the night-of date:"));
children.push(code("M_81 RASA8 ASI2600MCAir 2026-04-19"));
children.push(code("NGC_1499 Pleiades111 ASI2600MCAir 2026-02-26"));
children.push(p("The date follows the night-of (civil night) convention: every frame captured between local noon on day D and local noon on day D+1 belongs to the session dated D. A night whose only exposures start after midnight still carries the evening's date, and every rig operating on one observing night shares one date — which keeps multi-rig nights, shared flat sets, and the importer's duplicate detection aligned on a single key. The capture software's filename timestamps are device-local on every platform in use (ASIAir and NINA alike), so the date falls straight out of the stamp with a noon cutoff; only raw card dumps, whose filenames carry no timestamp, date by file modification time through a configured home timezone."));
...
children.push(p("One cross-link keeps these lists honest. A stack can fail and leave no trace in the session folder — the working files live on the processing machine — so to the scanner the session simply looks ready to run again. The durable record of such a failure is the note written that night: any session with open reprocessing to-dos in its notes file carries a see-notes marker in the cull, integrate, and edit lists, putting “read the notes first” in the exact row that would otherwise invite a blind re-run. The marker clears itself when the to-do line is deleted."));
