# A simple system for local astrophotography data organization

*An organizer and tracker that derives processing state directly from the files.*

*A dashboard to see all your captured data — target lists with integration time and the current image-processing state of every target.*

*A few scripts using controlled vocabularies, a session-keyed folder structure, and consistent calibration make keeping your library a simple task.*

**📄 [Read the paper (PDF)](Astrophotography_v2_organization_paper.pdf)**
...
The paper documents the conventions, the data structures, and the toolchain, and closes with appendices for readers who want to adopt the system against an existing library — the code here, so the scripts can be run, not just read.
