...
ACTIONS = ("capture", "coverage", "masters", "cull", "integrate", "restack", "edit")

QUERIES = {
    "cull": (
        "Sessions to cull (captured, not yet reviewed)",
        """
    ...
    "integrate": (
        "Sessions to integrate (culled, not yet stacked)",
        """
        SELECT s.target_id AS target, s.session_date AS date, s.scope, s.sensor,
    ...
    "edit": (
        "Finished images to edit (integrated, not edited)",
        """
    ...
    "restack": (
        "Integrations to restack (new data since last stack)",
        """
    ...
    "capture": (
        "Targets to capture more of (under goal, or planned)",
        """
    ...
    "masters": (
        "Calibration sets to turn into masters (bias/dark, no master; "
...
def fetch(cur, key):
    ...
    data = [
        [fmts[h](v) if h in fmts else v for h, v in zip(headers, row)] for row in cur.fetchall()
    ]
...
def print_table(headers, data):
    ...
    if not data:
        ...
        return
    widths = [
        max(len(str(h)), max(len(str(r[i]) if r[i] is not None else "") for r in data))
    ...
    for r in data:
        print(
            "  "
            + "  ".join((str(v) if v is not None else "").ljust(widths[i]) for i, v in enumerate(r))
        )
...
def main():
    ...
    ap.add_argument("action", nargs="?", default="summary", choices=("summary", "all") + ACTIONS)
