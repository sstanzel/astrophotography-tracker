...
children.push(new Paragraph({ alignment: AlignmentType.CENTER,
  children: [new TextRun({ text: "Controlled vocabularies, a session-keyed folder scaffold, a shared calibration library, and a tracker that derives every processing state from the files themselves", font: ARIAL, italics: true, size: 26 })],
  spacing: { after: 240 } }));
children.push(new Paragraph({ alignment: AlignmentType.CENTER,
...
children.push(bulletRich([new TextRun({ text: "One scaffold per night. ", bold: true, font: ARIAL }), new TextRun({ text: "Post Haste (Digital Rebellion's free folder-templating application) materializes the same per-session subtree every time, so PixInsight and PIMagic Studio always find what they need.", font: ARIAL })]));
...
children.push(p("A session folder is one acquisition night with one telescope and one camera. The name is four whitespace-delimited tokens — the target identifier with underscores joining its parts, the telescope, the camera, and the local calendar date the session started:"));
...
children.push(p("Not every astronomy session goes through the standard imaging rigs. Lunar imaging through a visual telescope, comets and asteroids, aurora and time-lapse work with DSLR and mirrorless bodies, and one-off test shoots share a property: they do not fit the four-token session grammar, and their frame counts would distort deep-sky integration statistics. They live in a small set of top-level buckets — Asteroids Comets, Moon Daytime, Moon Nighttime, a time-lapse bucket, and a miscellaneous bucket — that are recorded by the tracker but deliberately exempt from the naming conventions and excluded from deep-sky totals. Named-star sessions of single bright stars are not “other captures”: they use a standard imaging rig and follow the normal grammar."));
...
children.push(...figure(path.join(FIGDIR, "fig1_dashboard_top.png"), 1400, 705,
  "Figure 1 — the dashboard's summary layer: headline cards, integration by year, and top targets by lifetime hours."));
...
children.push(p("New sessions are staged in a holding folder before being filed into the library. A read-only preflight script checks each staged session before it moves: the folder name parses into four valid tokens, the target folder exists (or the registry knows it), the rig combination is registered, the frames inside parse, and the move would not collide with an existing session. It prints a verdict per session and touches nothing. The same script resolves the _adjacent suffix and reports what a staged piggyback session will attach to. Between the preflight at the front door and the validation pass behind it, a malformed name has no quiet way into the library — which is what keeps a convention alive longer than enthusiasm does."));
